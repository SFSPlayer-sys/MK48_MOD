...
pub fn mk48_ui(props: &PropertiesWrapper<UiProps>) -> Html {
    ...
    let on_play = Callback::from(|_alias| {
        let wss_url = web_sys::window()
            ...
            .unwrap_or_else(|| "wss://mk48.io".to_string());
        let _ = js_sys::Reflect::get(&web_sys::window().unwrap(), &"mk48_connect".into())
            .and_then(|f| f.dyn_into::<js_sys::Function>())
            .map(|f| f.call1(&JsValue::NULL, &JsValue::from_str(&wss_url)));
    });
