...
#[wasm_bindgen]
pub fn mk48_connect(wss_url: Option<String>) {
    use wasm_bindgen::JsCast;
    use web_sys::window;
    let url = if let Some(url) = wss_url {
        url
    } else {
        // 浏览器弹窗输入
        let win = window().unwrap();
        let input = win.prompt_with_message("Enter WSS server address:").unwrap_or(None);
        input.unwrap_or_else(|| "wss://mk48.io".to_string())
    };
    web_sys::console::log_1(&format!("[mk48_connect] Connecting to: {}", url).into());
    // 这里假设有run_with_url，实际项目需补充run_with_url实现
    Mk48Game::run_with_url(url);
}
